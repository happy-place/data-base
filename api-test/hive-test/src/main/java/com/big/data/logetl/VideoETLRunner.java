...
public class VideoETLRunner implements Tool {
    ...
    public int run(String[] args) throws Exception {
        conf = this.getConf();
        // hdfs://localhost:9000/tmp/youtube/user
        if (args.length==0) {
            args = new String[]{"hdfs://localhost:9000/tmp/youtube/video_ori", "hdfs://localhost:9000/tmp/youtube/video"};
        }

        conf.set("inpath", args[0]); // hdfs://localhost:9000/tmp/youtube/video_ori
        conf.set("outpath", args[1]);// hdfs://localhost:9000/tmp/youtube/video
